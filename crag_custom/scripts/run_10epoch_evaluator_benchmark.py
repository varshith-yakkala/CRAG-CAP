...
def main():
    print("==================================================================", flush=True)
    print("STARTING 10-EPOCH T5 EVALUATOR FULL BENCHMARK (1,385 QUERIES)", flush=True)
    print("==================================================================", flush=True)
    ...
    # Output Directory for 10-Epoch Benchmark
    out_dir = r"c:\Users\varsh\OneDrive\Desktop\2-2\capstone\results\full_benchmark_10epochs"
    ...
    jsonl_path = os.path.join(out_dir, "full_popqa_10epochs_predictions.jsonl")
    ...
    # 2. Instantiate 10-Epoch T5 Evaluator & Controller
    new_evaluator_path = r"c:\Users\varsh\OneDrive\Desktop\2-2\capstone\t5_evaluator_model\t5_evaluator_10epochs_final"
    print(f"\n[INIT] Initializing T5RetrievalEvaluator with 10-Epoch Checkpoint: {new_evaluator_path}", flush=True)
    evaluator_10ep = T5RetrievalEvaluator(model_path=new_evaluator_path)
    ...
    with open(jsonl_path, "a", encoding="utf-8") as f_out:
        for idx, item in enumerate(dataset, start=1):
            ...
            pipeline = CRAGPipeline(retriever=query_retriever, evaluator=evaluator_10ep, controller=controller)
    ...
    # 5. Compute 10-Epoch Benchmark Metrics
    ...
    metrics_summary = {
        "evaluator_version": "10-Epoch T5-Small Evaluator (t5_evaluator_10epochs_final)",
    ...
    metrics_json_path = os.path.join(out_dir, "full_popqa_10epochs_metrics.json")
    ...
    print(f"Final 10-Epoch PopQA Answer Accuracy: {match_acc:.2f}% ({correct_matches}/{len(existing_records)})", flush=True)
